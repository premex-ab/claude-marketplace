...
def render_readme(
    ...
    """Render a README.md for an auto-synced android-* plugin.

    Kept intentionally short — the SKILL.md is the authoritative content. The
    README is a packaging wrapper that explains the install command and points
    at upstream for authoritative updates.
    """
    references_line = (
        f"- `skills/{skill_name}/references/` — supporting docs referenced from the skill\n"
        if has_references
        else ""
    )
...
**Upstream:** [`android/skills/{upstream_rel_path}`](https://github.com/android/skills/tree/main/{upstream_rel_path}) — mirrored and split into a one-plugin-per-skill layout so you can install skills individually. Auto-synced daily from upstream; the SKILL.md is Google-authored.

## Install

```bash
/plugin install {plugin_name}@premex-plugins
```

## What's in the box

- `skills/{skill_name}/SKILL.md` — the skill definition
{references_line}- `LICENSE.txt` — upstream Apache-2.0 license text
...
def write_plugin(skill_md: Path) -> str:
    ...
    has_references = (dest_skill / "references").is_dir()
    upstream_rel_path = str(skill_dir.relative_to(UPSTREAM))
    (plugin_dir / "README.md").write_text(
        render_readme(
            plugin_name=plugin_name,
            skill_name=skill_name,
            description=description,
            upstream_rel_path=upstream_rel_path,
